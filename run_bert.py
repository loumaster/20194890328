...
#为训练、验证集创建的样本对象，包含id，content，title，label
...
def read_examples(input_file, is_training):
    ...
    #一个字典，用来存储InputExample对象，每为一行为一个对象
    examples=[]
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,split_num,
    ...
    for example_index, example in enumerate(examples):
        ...
        #tokenizer分词器，结果是content、title的分词，返回一个列表
        ...
        for step in bar:
            ...
            if (nb_tr_steps + 1) % args.gradient_accumulation_steps == 0:
                if args.fp16:
                    ...
                    lr_this_step = args.learning_rate * args.warmup_linear.get_lr(global_step, args.warmup_proportion)
